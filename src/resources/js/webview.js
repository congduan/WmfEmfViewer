...
function formatData(data, typeName) {
  ...
  switch (typeName) {
    ...
    case 'META_CREATEBRUSHINDIRECT':
      return `[复杂参数]`;
    case 'META_TEXTOUT':
      // META_TEXTOUT 结构: 16位x, 16位y, 16位选项, 16位长度, 文本数据
      if (data.length >= 8) {
        const x = (data[0] | (data[1] << 8)) << 16 >> 16;
        const y = (data[2] | (data[3] << 8)) << 16 >> 16;
        const length = data[6] | (data[7] << 8);
        if (data.length >= 8 + length) {
          try {
            // 尝试将文本数据转换为字符串
            const textData = data.slice(8, 8 + length);
            const text = new TextDecoder('ascii').decode(textData);
            return `(${x}, ${y}) "${text}"`;
          } catch (e) {
            return `(${x}, ${y}) [文本数据]`;
          }
    ...
    case 'META_EXTTEXTOUT':
      // 调试信息
      console.log('META_EXTTEXTOUT data length:', data.length);
      console.log('META_EXTTEXTOUT data:', Array.from(data));

      // 尝试多种方法解析文本数据
      // 方法1: 标准结构 (保留2字节 + 选项2字节 + RECT 8字节 + 长度2字节 = 14字节)
      if (data.length >= 14) {
        const length = data[12] | (data[13] << 8);
        console.log('Method 1 - Length from header:', length);

        if (data.length >= 14 + length && length > 0) {
          try {
            const textData = data.slice(14, 14 + length);
            console.log('Method 1 - Text data:', Array.from(textData));
            const text = new TextDecoder('ascii').decode(textData).replace(/\0/g, '').replace(/[\x00-\x1F]/g, '');
            console.log('Method 1 - Decoded text:', text);
            if (text.length > 0) {
              return `"${text}"`;
            }
          } catch (e) {
      ...
      // 假设文本从第12字节开始
      if (data.length >= 12) {
        console.log('Method 2 - Trying from offset 12');
        try {
          const textData = data.slice(12);
          const text = new TextDecoder('ascii').decode(textData).replace(/\0/g, '').replace(/[\x00-\x1F]/g, '');
          console.log('Method 2 - Decoded text:', text);
          if (text.length > 0) {
            return `"${text}"`;
          }
        } catch (e) {
      ...
      // 方法3: 尝试从第8字节开始
      if (data.length >= 8) {
        console.log('Method 3 - Trying from offset 8');
        try {
          const textData = data.slice(8);
          const text = new TextDecoder('ascii').decode(textData).replace(/\0/g, '').replace(/[\x00-\x1F]/g, '');
          console.log('Method 3 - Decoded text:', text);
          if (text.length > 0) {
            return `"${text}"`;
          }
        } catch (e) {
      ...
      // 方法4: 尝试整个数据的ASCII解码
      console.log('Method 4 - Trying entire data');
      try {
        const text = new TextDecoder('ascii').decode(data).replace(/\0/g, '').replace(/[\x00-\x1F]/g, '');
        console.log('Method 4 - Decoded text:', text);
        if (text.length > 0) {
          return `"${text}"`;
        }
      } catch (e) {
    ...
    case 'EMR_EXTTEXTOUTA':
      // EMR_EXTTEXTOUTA 结构: 各种参数后跟ASCII文本
      if (data.length >= 40) {
        // 跳过前面的参数，找到文本数据
        // 实际结构复杂，这里简单处理
        try {
          // 从偏移量40开始尝试解析文本
          const textData = data.slice(40);
          const text = new TextDecoder('ascii').decode(textData).replace(/\0/g, '');
          if (text.length > 0) {
            return `"${text}"`;
          }
        } catch (e) {
    ...
    case 'EMR_EXTTEXTOUTW':
      // EMR_EXTTEXTOUTW 结构: 各种参数后跟Unicode文本
      if (data.length >= 40) {
        // 跳过前面的参数，找到文本数据
        try {
          // 从偏移量40开始尝试解析Unicode文本
          const textData = data.slice(40);
          const text = new TextDecoder('utf-16le').decode(textData).replace(/\0/g, '');
          if (text.length > 0) {
            return `"${text}"`;
          }
        } catch (e) {
    ...
    case 'EmfPlusDrawString':
      // EmfPlusDrawString 结构: 包含文本数据
      if (data.length >= 24) {
        try {
          // 跳过前面的参数，找到文本数据
          const textData = data.slice(24);
          const text = new TextDecoder('utf-16le').decode(textData).replace(/\0/g, '');
          if (text.length > 0) {
            return `"${text}"`;
          }
...
function previewWMF(base64Data) {
    try {
        ...
        // 绘制WMF文件，传入view尺寸
        drawer.draw(result, { viewWidth, viewHeight });
