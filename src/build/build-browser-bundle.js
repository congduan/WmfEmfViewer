...
function buildBrowserBundle() {
  ...
  // 读取文件类型检测模块
  const fileTypeDetectorPath = path.join(__dirname, '..', 'utils', 'fileTypeDetector.js');
  ...
  // 读取坐标转换模块
  const coordinateTransformerPath = path.join(__dirname, '..', 'utils', 'coordinateTransformer.js');
  ...
  // 读取SVG输出上下文模块
  const svgContextPath = path.join(__dirname, '..', 'modules', 'svgContext.js');
  let svgContextContent = fs.readFileSync(svgContextPath, 'utf8');
  svgContextContent = removeNodeModuleSyntax(svgContextContent);
