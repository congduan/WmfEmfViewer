...
// 模拟浏览器环境加载打包产物（需先 npm run build:bundle）
global.window = {};
...
const WmfParser = window.WmfParser;
...
class MockContext {
  ...
  // 其余 Canvas API 用 no-op 保证绘制流程不中断
  scale() { }
  save() { }
  restore() { }
  translate() { }
...
async function testSampleWmf() {
  ...
  try {
    ...
    // 转换为Uint8Array（注意带上 byteOffset/byteLength，Buffer 可能共享内存池）
    const wmfData = new Uint8Array(fileBuffer.buffer, fileBuffer.byteOffset, fileBuffer.byteLength);
